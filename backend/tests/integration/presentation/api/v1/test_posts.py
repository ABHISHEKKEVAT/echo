...
async def test_create_post_persists_authenticated_user(async_client: AsyncClient, db_session: AsyncSession):
    token, _ = await issue_token_pair(async_client, "posts_int_1@example.com", "posts_int_1")
...
async def test_list_my_posts_filters_only_current_user(async_client: AsyncClient, db_session: AsyncSession):
    token, _ = await issue_token_pair(async_client, "posts_int_2@example.com", "posts_int_2")
    other = User(
        id=uuid6.uuid7(),
...
async def test_list_user_posts_filters_target_user(async_client: AsyncClient, db_session: AsyncSession):
    token, _ = await issue_token_pair(async_client, "posts_int_3@example.com", "posts_int_3")
    target = User(
        id=uuid6.uuid7(),
...
async def test_list_following_feed_uses_accepted_friend_relations(async_client: AsyncClient, db_session: AsyncSession):
    token, _ = await issue_token_pair(async_client, "posts_int_4@example.com", "posts_int_4")
...
async def test_attachments_loaded_without_subtype_joins(async_client: AsyncClient, db_session: AsyncSession):
    token, _ = await issue_token_pair(async_client, "posts_int_5@example.com", "posts_int_5")
...
async def test_signed_attachment_url_present_with_provider(async_client: AsyncClient, db_session: AsyncSession):
    token, _ = await issue_token_pair(async_client, "posts_int_6@example.com", "posts_int_6")
...
async def test_cursor_ordering_desc(async_client: AsyncClient, db_session: AsyncSession):
    token, _ = await issue_token_pair(async_client, "posts_int_7@example.com", "posts_int_7")
...
async def test_signed_url_expiry_within_5_minutes(async_client: AsyncClient, db_session: AsyncSession):
    ...
    token, _ = await issue_token_pair(async_client, "posts_int_8@example.com", "posts_int_8")
...
async def test_like_comment_and_activity_notifications_flow(async_client: AsyncClient, db_session: AsyncSession):
    owner_token, _ = await issue_token_pair(async_client, "posts_int_9_owner@example.com", "posts_int_9_owner")
    actor_token, _ = await issue_token_pair(async_client, "posts_int_9_actor@example.com", "posts_int_9_actor")
    ...
    latest_activity = (
        await db_session.execute(select(PostActivityNotification).order_by(PostActivityNotification.created_at.desc()).limit(1))
    ).scalar_one()

...
async def test_follow_request_accept_and_status(async_client: AsyncClient):
    token_a = await issue_access_token(async_client, "friends_a@example.com", "friends_a")
    token_b = await issue_access_token(async_client, "friends_b@example.com", "friends_b")
    ...
    assert any(item["user_id"] == target_id and item["username"] == "friends_b" for item in friends_resp.json())

...
class EmailAuthRepository extends ChangeNotifier implements AuthRepository {
  ...
  String _v1Url(String path) {
    final base = _echoBaseUrl.replaceAll(RegExp(r'/+$'), '');
    final normalizedPath = path.startsWith('/') ? path : '/$path';
    if (base.endsWith('/v1') && normalizedPath.startsWith('/v1/')) {
      return '$base${normalizedPath.substring(3)}';
    }
  ...
  Future<PendingVerification> resendVerificationCode(String email) async {
    try {
      final response = await _dio.post(
        _v1Url('/v1/auth/resend-verification'),
        data: {'email': email},
      );
  ...
  Future<void> loginWithSoundCloud() async {
    try {
      ...
      final response = await _dio.post(
        _v1Url('/v1/auth/soundcloud/token'),

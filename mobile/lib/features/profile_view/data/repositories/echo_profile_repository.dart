...
class EchoProfileRepository implements ProfileRepository {
  EchoProfileRepository({
    ...
    Dio? dio,
       ...
       _api = AuthenticatedApiClient(
         ...
         onMissingToken: () => const ProfileAuthException('No active session'),
         dio: dio,
       );
  ...
  Future<ProfileHeader> getOwnProfile() async {
    try {
      final response = await _api.get(_api.path('/v1/me'));
  ...
  Future<ProfileHeader> getUserProfile(String userId) async {
    try {
      final response = await _api.get(_api.path('/v1/users/$userId'));
      return _mapProfile(response.data as Map<String, dynamic>);
    } on DioException catch (e) {
      _translateError(e);
    }
  ...
  Future<ProfilePostsPage> getUserPosts(
    ...
    try {
      ...
      if (cursor != null) queryParams['cursor'] = cursor;
      final response = await _api.get(
        _api.path('/v1/user/$userId/posts'),
        queryParameters: queryParams,
      );
      return _mapPostsPage(response.data as Map<String, dynamic>);
    } on DioException catch (e) {
  ...
  Future<List<ProfilePostComment>> listPostComments(String postId) async {
    try {
      final response = await _api.get(_api.path('/v1/posts/$postId/comments'));
  ...
  Future<ProfilePostComment> createPostComment(
    ...
    try {
      final response = await _api.post(
        _api.path('/v1/posts/$postId/comments'),
  ...
  Future<FollowRelationStatus> getFollowStatus(String userId) async {
    try {
      final response = await _api.get(_api.path('/v1/friends/$userId/status'));
      final json = response.data as Map<String, dynamic>;
  ...
  Future<void> sendFollowRequest(String userId) async {
    try {
      await _api.post(_api.path('/v1/friends/$userId/request'));
    } on DioException catch (e) {
      _translateError(e);
    }
  }
  ...
  Future<void> acceptFollowRequest(String userId) async {
    try {
      await _api.post(_api.path('/v1/friends/$userId/accept'));
    } on DioException catch (e) {
      _translateError(e);
    }
  }

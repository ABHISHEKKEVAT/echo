...
class EchoMusicSearchRepository implements MusicSearchRepository {
  EchoMusicSearchRepository({
    ...
    Dio? dio,
         ...
         onMissingToken: () =>
             const MusicSearchAuthException('No active session'),
         dio: dio,
       );
  ...
  Future<MusicSearchResultGroup> search(String query) async {
    try {
      final usersFuture = _api
          .get(
            _api.path('/v1/users/search'),
            queryParameters: {'q': query, 'limit': 20},
          )
          .then((response) => _mapUsersResponse(response.data))
          .catchError((_) => const <UserSearchResult>[]);
      final musicResponse = await _api.post(
        _api.path('/v1/search/music'),
        data: {'q': query},
      );
